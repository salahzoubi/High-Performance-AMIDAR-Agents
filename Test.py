                ...
                for i in range(10000):
                    ...
                    if i % 3 == 0:
                        ...
                        with AmidarIntervention(tb) as intervention:
                            ...
                            vals = [calc_distance(player_pos, enemy_0)] #, calc_distance(player_pos, enemy_1), calc_distance(player_pos, enemy_2), calc_distance(player_pos, enemy_3), calc_distance(player_pos, enemy_4)
                            ...
                            if i > 0 and closest_dist <= k and not enemy_avoided:
                                ...
                                for dir,length in find_dir_length(possible, player_pos, intervention):
                                    ...
                                    calced = calc_distance1(projected[0], projected[1], enemies[enemy_idx][0],  enemies[enemy_idx][1])

                                    if calced > max_dist and calced != get_opposite_direction(last_dir):
                                        max_dist = calc_distance1(projected[0], projected[1],enemies[enemy_idx][0],  enemies[enemy_idx][1])
                                        direction = dir
                                        # print("{}. we're in the worst one".format(i))
                                ...
                                if checked[1] == 0: #that is, the agent can't see anywhere
                                    ...
                                    direction = checked[0]
                                    move_to_take.append(direction)
                                    # print("{}. were in elif1".format(i))


                                elif agent_stuck((player_pos.tx, player_pos.ty), last_pos):
                                    direction = checked[0]
                                    move_to_take.append(direction)
                                    # print("{}. we're in elif 2".format(i))


                                else:
                                    direction = last_dir
                                    move_to_take.append(direction)
                                    # print("{}. were in the else".format(i))
